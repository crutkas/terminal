...
void TerminalApiTest::KittyAnimationSurvivesFontAndRendererRefresh()
{
    ...
    // Re-ask each time round rather than waiting out one long timeout, for the same
    // reason KittyRenderFixture::_pump does: a loaded machine can lose a single request.
    for (auto attempt = 0; attempt < 15 && !advanced; ++attempt)
    {
        const auto previousFrame = fixture.engine.PrepareForNextFrame();
        fixture.renderer.NotifyPaintFrame();
        if (!fixture.engine.WaitForFrameAfter(previousFrame, 1000))
        {
            continue;
        }
