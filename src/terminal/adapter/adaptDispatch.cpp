    ...
    _pages{ api, renderer }
{
    if (_renderer)
    {
        try
        {
            auto target = std::make_shared<KittyAnimationTimerTarget>();
            target->dispatch = this;
            _kittyAnimationTimer = _renderer->RegisterTimer("kitty animation", [weakTarget = std::weak_ptr{ target }](Renderer&, const TimerHandle) {
                if (const auto target = weakTarget.lock())
                {
                    const std::scoped_lock lock{ target->mutex };
                    if (target->dispatch)
                    {
                        target->dispatch->_advanceKittyAnimations(std::chrono::steady_clock::now());
                    }
                }
            }, target);
            _kittyAnimationTimerTarget = std::move(target);
        }
        CATCH_LOG();
    }
}
...
{
    if (_kittyAnimationTimerTarget)
    {
        const std::scoped_lock lock{ _kittyAnimationTimerTarget->mutex };
        _kittyAnimationTimerTarget->dispatch = nullptr;
    }
}
...
void AdaptDispatch::_WriteToBuffer(const std::wstring_view string)
{
    ...
    const auto hasKittyPlaceholders = string.find(KittyPlaceholderCodePointHigh) != std::wstring_view::npos;
    ...
    while (!state.text.empty())
    {
        ...
        if (hasKittyPlaceholders && textPositionAfter > textPositionBefore)
        {
            ...
            if (segment.find(KittyPlaceholderCodePointHigh) != std::wstring_view::npos)
            {
                _renderKittyPlaceholders(segment, cursorPosition.y, state.columnBegin);
            }
        }
    ...
    while (pos <= control.size())
    {
        ...
        if (eq == 1 && pair.size() > 2)
        {
            ...
            case L'v':
                ...
                break;
            case L'c':
                c.cols = _ParseKittyUint(value);
                break;
            case L'r':
                c.rows = _ParseKittyUint(value);
                break;
            case L'x':
                c.srcX = _ParseKittyUint(value);
                break;
            case L'y':
                c.srcY = _ParseKittyUint(value);
                break;
            case L'w':
                c.srcW = _ParseKittyUint(value);
                break;
            case L'h':
                c.srcH = _ParseKittyUint(value);
                break;
            case L'X':
                c.upperX = _ParseKittyUint(value);
                break;
            case L'Y':
                c.upperY = _ParseKittyUint(value);
                break;
            case L'o':
...
void AdaptDispatch::_HandleKittyGraphics(const std::wstring_view control, const std::string_view payload, const bool payloadValid, const bool payloadTooLarge)
{
    ...
    const auto repeatsActiveAction = _kittyChunkActive &&
...
void AdaptDispatch::_ProcessKittyCommand(const KittyControl& command, const std::string_view payload, const bool payloadValid, const bool payloadTooLarge)
{
    ...
    const auto displayKittyPlacement = [&](const uint32_t targetImageId, const KittyImage& image) {
        ...
        auto layerId = _kittyNextLayerId++;
        if (layerId == 0)
        {
            layerId = _kittyNextLayerId++;
        }
        auto priorPlacement = std::optional<KittyPlacement>{};
        if (placementId != 0)
        {
        ...
        const auto removePriorPlacement = [&]() {
            if (priorPlacement)
            {
                _eraseKittyPlacementCells(*priorPlacement);
        ...
        const auto registerPlacement = [&](const KittyPlacement& placement) {
            ...
            {
                const auto& victim = _kittyAnonymousPlacements.front();
                _eraseKittyPlacementCells(victim);
                if (victim.isVirtual)
                {
                    _kittyVirtualIds.erase({ victim.imageId, 0u });
                }
        ...
        if (command.haveParent)
        {
            ...
            if (!_kittyPlacementFitsMemory(image, targetImageId, layerId, command.cols, command.rows, command.srcX, command.srcY, command.srcW, command.srcH, command.zIndex, childAnchor))
            {
                success = false;
            ...
            try
            {
                drawn = _placeKittyImage(image, false, targetImageId, layerId, command.cols, command.rows, command.srcX, command.srcY, command.srcW, command.srcH, command.zIndex, childAnchor);
            }
            catch (const std::bad_alloc&)
            {
                success = false;
        ...
        if (!_kittyPlacementFitsMemory(image, targetImageId, layerId, command.cols, command.rows, command.srcX, command.srcY, command.srcW, command.srcH, command.zIndex))
        {
            success = false;
            code = L"ENOMEM:image layer memory limit exceeded";
            return;
        ...
        try
        {
            drawn = _placeKittyImage(image, moveCursor, targetImageId, layerId, command.cols, command.rows, command.srcX, command.srcY, command.srcW, command.srcH, command.zIndex);
        }
        catch (const std::bad_alloc&)
        {
            success = false;
    ...
    const auto storeKittyVirtualPlacement = [&](const uint32_t targetImageId, const KittyImage& image) {
        ...
        _storeKittyVirtualPlacement(targetImageId, placementId, image, command.cols, command.rows, command.srcX, command.srcY, command.srcW, command.srcH, command.zIndex, layerId);
        ...
        const auto virtualPlacement = _kittyVirtualIds.find({ targetImageId, placementId });
        if (virtualPlacement == _kittyVirtualIds.end())
        {
            return;
        }
        KittyPlacement placement;
    ...
    else
    {
        ...
        case L'q': // query: validate the request the same way but do not store
        {
            ...
            if (action != L'q')
            {
                ...
                {
                    std::erase_if(_kittyVirtualIds, [&](const auto& entry) {
        ...
        case L'c': // compose animation frames
        {
            ...
            {
                success = _processKittyAnimationFrame(command, payload, payloadValid, payloadTooLarge, targetId, code);
            }
            else if (action == L'a')
            {
                success = _processKittyAnimationControl(command, targetId, code);
            }
            else
            {
                success = _processKittyFrameComposition(command, targetId, code);
            }
            break;
        }
        case L'd': // delete
        {
            ...
            case L'F':
            {
                ...
                if (targetId != 0)
                {
                    _deleteKittyAnimationFrames(targetId, command.rows == 0 ? 1 : command.rows, freeData);
                }
                break;
            }
            default:
...
void AdaptDispatch::_eraseKittyImage(const uint32_t id)
{
    ...
    _kittyTotalPixelBytes -= it->second.PixelBytes();
...
try
{
    ...
    if (_renderer && _kittyAnimationTimer)
    {
        _renderer->StopTimer(*_kittyAnimationTimer);
    }
    auto page = _pages.ActivePage();
    ...
    for (auto row = 0; row < page.Bottom(); ++row)
    {
        ...
        {
            for (const auto owner : slice->ColumnOwners(column))
            {
...
RGBQUAD AdaptDispatch::_kittyRgbaColor(const uint32_t rgba) noexcept
{
...
void AdaptDispatch::_kittyCompositePixels(const std::span<RGBQUAD> destination, const std::span<const RGBQUAD> source, const bool replace) noexcept
{
...
size_t AdaptDispatch::_kittyFrameCount(const KittyImage& image) noexcept
{
    return image.pixels.empty() ? 0 : image.animationFrames.size() + 1;
}

std::vector<RGBQUAD>* AdaptDispatch::_kittyFramePixels(KittyImage& image, const uint32_t frameNumber) noexcept
{
    if (frameNumber == 1)
    {
        return &image.pixels;
...
const std::vector<RGBQUAD>* AdaptDispatch::_kittyFramePixels(const KittyImage& image, const uint32_t frameNumber) noexcept
{
    if (frameNumber == 1)
    {
        return &image.pixels;
...
int32_t* AdaptDispatch::_kittyFrameGap(KittyImage& image, const uint32_t frameNumber) noexcept
{
    if (frameNumber == 1)
    {
...
try
{
    auto imageIt = _kittyImages.find(imageId);
    if (imageIt == _kittyImages.end())
    {
        code = L"ENOENT:image not found";
        return false;
    }
    ...
    if (command.format == 24 || command.format == 32)
    {
        ...
        framePixels = _decodeKittyPixels(command.format, bytes);
    }
    else
    {
        til::size decodedSize;
        if (bytes.empty() ||
            !_api.DecodeImageToBgra(bytes, framePixels, decodedSize) ||
            decodedSize.width <= 0 || decodedSize.height <= 0 ||
    ...
    if (editFrame == 0 && _kittyFrameCount(image) >= MaxKittyFramesPerImage)
    {
        code = L"ENOSPC:frame count limit exceeded";
        return false;
    }
    if (editFrame != 0 && !_kittyFramePixels(image, editFrame))
    {
        code = L"ENOENT:frame not found";
        return false;
    }
    if (editFrame == 0 && backgroundFrame != 0 && !_kittyFramePixels(image, backgroundFrame))
    {
    ...
    if (editFrame == 0)
    {
        ...
        const auto protectAncestors = [&](std::pair<uint32_t, uint32_t> parent) {
            for (auto depth = 0; depth < MaxKittyPlacementDepth; ++depth)
            {
                ...
                const auto placement = _kittyPlacements.find(parent);
                if (placement == _kittyPlacements.end() || !placement->second.hasParent)
                {
                    break;
                }
    ...
    if (editFrame != 0)
    {
        canvas = *_kittyFramePixels(imageIt->second, editFrame);
    }
    else if (backgroundFrame != 0)
    {
        canvas = *_kittyFramePixels(imageIt->second, backgroundFrame);
    }
    else
    {
        canvas.assign(imageIt->second.pixels.size(), _kittyRgbaColor(command.upperY));
    }
    ...
    for (uint32_t row = 0; row < frameHeight; ++row)
    {
        ...
        _kittyCompositePixels(std::span{ canvas }.subspan(dstOffset, frameWidth),
    ...
    if (editFrame == 0)
    {
        ...
        {
            if (_kittyImages.count(victimId) == 0)
            {
                continue;
            }
        ...
        imageIt = _kittyImages.find(imageId);
        WI_ASSERT(imageIt != _kittyImages.end());
    }
    ...
    if (editFrame != 0)
    {
        *_kittyFramePixels(storedImage, editFrame) = std::move(canvas);
        changedFrame = editFrame;
        if (command.haveZ && command.zIndex != 0)
        {
            *_kittyFrameGap(storedImage, editFrame) = command.zIndex;
        }
    }
    else
    {
        KittyAnimationFrame frame;
        ...
        _kittyTotalPixelBytes += frameBytes;
    ...
    if (storedImage.animationState == 2 && storedImage.waitingForFrames)
    {
        ...
        _advanceKittyImage(imageId, storedImage, now);
    }
    ...
    {
        _scheduleKittyAnimation(imageId, storedImage, now);
    }
...
bool AdaptDispatch::_processKittyAnimationControl(const KittyControl& command, const uint32_t imageId, std::wstring_view& code)
{
    const auto imageIt = _kittyImages.find(imageId);
    if (imageIt == _kittyImages.end())
    {
        code = L"ENOENT:image not found";
        return false;
    ...
    if (command.cols != 0 && !_kittyFramePixels(image, command.cols))
    {
        code = L"ENOENT:frame not found";
        return false;
    }
    if (command.rows != 0 && !_kittyFrameGap(image, command.rows))
    {
        code = L"ENOENT:frame not found";
        return false;
    }
    ...
    {
        *_kittyFrameGap(image, command.rows) = command.zIndex;
    ...
    {
        _scheduleKittyAnimation(imageId, image, std::chrono::steady_clock::now());
    }
    _scheduleKittyAnimationTimer();
    return true;
}

bool AdaptDispatch::_processKittyFrameComposition(const KittyControl& command, const uint32_t imageId, std::wstring_view& code)
try
{
    const auto imageIt = _kittyImages.find(imageId);
    if (imageIt == _kittyImages.end())
    ...
    const auto* source = _kittyFramePixels(image, sourceFrame);
    auto* destination = _kittyFramePixels(image, destinationFrame);
    ...
    for (uint32_t row = 0; row < height; ++row)
    {
        ...
        _kittyCompositePixels(std::span{ *destination }.subspan(dstOffset, width),
...
void AdaptDispatch::_scheduleKittyAnimation(const uint32_t imageId, KittyImage& image, const std::chrono::steady_clock::time_point now)
{
    ...
    const auto* gap = _kittyFrameGap(image, image.currentFrame);
    if (!gap)
    {
    ...
    else
    {
        ...
        _advanceKittyImage(imageId, image, now);
    }
}

bool AdaptDispatch::_advanceKittyImage(const uint32_t imageId, KittyImage& image, const std::chrono::steady_clock::time_point now)
{
    if (image.animationState != 2 && image.animationState != 3)
    {
        image.nextFrameTime = {};
    ...
    const auto frameCount = gsl::narrow_cast<uint32_t>(_kittyFrameCount(image));
    if (frameCount < 2)
    {
...
void AdaptDispatch::_advanceKittyAnimations(const std::chrono::steady_clock::time_point now)
{
    for (auto& [imageId, image] : _kittyImages)
    {
        if ((image.animationState == 2 || image.animationState == 3) &&
...
void AdaptDispatch::_scheduleKittyAnimationTimer()
{
    if (!_renderer || !_kittyAnimationTimer)
    {
        return;
    ...
    for (const auto& [id, image] : _kittyImages)
    {
        static_cast<void>(id);
        if ((image.animationState == 2 || image.animationState == 3) &&
            image.nextFrameTime != std::chrono::steady_clock::time_point{})
    ...
    const auto now = std::chrono::steady_clock::now();
    auto delay = std::max(earliest - now, std::chrono::steady_clock::duration{ std::chrono::milliseconds(1) });
    // Renderer timers deliberately assert on hour-scale delays. Wake periodically
    // for hostile/very large gaps, then reschedule until the true deadline.
    delay = std::min(delay, std::chrono::steady_clock::duration{ std::chrono::seconds(30) });
    _renderer->StartTimer(*_kittyAnimationTimer, std::chrono::duration_cast<TimerDuration>(delay));
}

void AdaptDispatch::_deleteKittyAnimationFrames(const uint32_t imageId, const uint32_t frameNumber, const bool freeData)
{
    const auto imageIt = _kittyImages.find(imageId);
    if (imageIt == _kittyImages.end())
    {
    ...
    const auto remaining = gsl::narrow_cast<uint32_t>(_kittyFrameCount(image));
...
bool AdaptDispatch::_kittyPlacementFitsMemory(const KittyImage& image, const uint32_t imageId, const uint64_t layerId, const uint32_t cols, const uint32_t rows, const uint32_t srcX, const uint32_t srcY, const uint32_t srcW, const uint32_t srcH, const int32_t zIndex, const std::optional<til::point> anchor) const noexcept
{
    ...
    const auto [targetW64, targetH64] = _kittyTargetPixels(cropW, cropH, cols, rows, cellWidth, cellHeight);
    ...
    auto available = ImageSlice::KittyLayerBytesAvailable();
    ...
    for (auto row = 0; row < rowSpan; ++row)
    {
        ...
        const auto required = slice && slice->CellSize() == clampedCellSize ?
                                  slice->KittyWriteMemoryUpperBound(columnBegin, columnEnd, zIndex, imageId, layerId) :
                                  emptySlice.KittyWriteMemoryUpperBound(columnBegin, columnEnd, zIndex, imageId, layerId);
...
til::size AdaptDispatch::_placeKittyImage(const KittyImage& image, const bool moveCursor, const uint32_t imageId, const uint64_t layerId, const uint32_t cols, const uint32_t rows, const uint32_t srcX, const uint32_t srcY, const uint32_t srcW, const uint32_t srcH, const int32_t zIndex, const std::optional<til::point> anchor)
{
    ...
    for (auto row = 0; row < rowSpan; ++row)
    {
        ...
        auto dstIterator = dstSlice->MutablePixels(columnBegin, columnEnd, zIndex, imageId, layerId);
        auto sourceIterator = dstSlice->MutableSourceIndices(columnBegin, columnEnd, zIndex, imageId, layerId);
        ...
        for (auto pixelRow = 0; pixelRow < cellHeight; ++pixelRow)
        {
            ...
            if (dstY < targetH)
            {
                ...
                for (auto pixelColumn = 0; pixelColumn < drawWidth; ++pixelColumn)
                {
                    ...
                    {
                        til::at(dstIterator, pixelColumn) = til::at(*framePixels, srcIndex);
                        til::at(sourceIterator, pixelColumn) = gsl::narrow_cast<uint32_t>(srcIndex);
                    }
                }
            }
...
AdaptDispatch::KittyTargetSize AdaptDispatch::_kittyTargetPixels(const int64_t cropW, const int64_t cropH, const uint32_t cols, const uint32_t rows, const int64_t cellWidth, const int64_t cellHeight) noexcept
{
...
void AdaptDispatch::_storeKittyVirtualPlacement(const uint32_t id, const uint32_t placementId, const KittyImage& image, const uint32_t cols, const uint32_t rows, const uint32_t srcX, const uint32_t srcY, const uint32_t srcW, const uint32_t srcH, const int32_t zIndex, const uint64_t layerId)
{
    constexpr uint32_t maxCells = 8192;
    const auto cellSize = _api.GetCellSize();
    ...
    // right/bottom edge. Matches _placeKittyImage so a U=1 placement samples the same sub-rect
    ...
    if (cropW <= 0 || cropH <= 0)
    {
        ...
        _kittyVirtualIds.erase({ id, placementId });
        return;
    }
    ...
    auto& placement = _kittyVirtualIds[{ id, placementId }];
...
void AdaptDispatch::_registerKittyPlacement(const KittyPlacement& placement)
{
    ...
    while (_kittyPlacements.size() > MaxKittyPlacements)
    {
        ...
        {
            _kittyVirtualIds.erase(victimKey);
        }
    ...
    while (!removed.empty() && guard-- > 0)
    {
        ...
        for (auto it = _kittyPlacements.begin(); it != _kittyPlacements.end();)
        {
            ...
            if (p.hasParent && p.parentImageId == parent.first && p.parentPlacementId == parent.second)
            {
                ...
                if (p.isVirtual)
                {
                    _kittyVirtualIds.erase(it->first);
                }
                removed.push_back(it->first);
                it = _kittyPlacements.erase(it);
            }
...
void AdaptDispatch::_deleteKittyPlacement(const uint32_t imageId, const uint32_t placementId, const bool freeData)
{
    ...
    _kittyVirtualIds.erase({ imageId, placementId });
...
void AdaptDispatch::_deleteKittyPlacementsByZ(const int32_t zIndex, const bool freeData, const std::optional<til::point> cell)
{
    ...
    if (cell)
    {
        if (cell->x >= 0 && cell->x < page.Width() && cell->y >= 0 && cell->y < page.Bottom())
        {
            ...
            if (slice)
            {
                for (const auto& identity : slice->KittyLayersAtZ(zIndex, cell->x))
                {
    ...
    const auto selected = [&](const KittyPlacement& placement) {
...
// Protocol: https://sw.kovidgoyal.net/kitty/graphics-protocol/#relative-placements
std::optional<til::point> AdaptDispatch::_deriveVirtualPlacementAnchor(const uint32_t imageId, const uint32_t placementId) const
{
    const auto virtualPlacement = _kittyVirtualIds.find({ imageId, placementId });
    if (imageId == 0 || virtualPlacement == _kittyVirtualIds.end())
    {
        return std::nullopt;
    }
    ...
    for (auto row = 0; row < page.Bottom(); ++row)
    {
        ...
        if (!slice || !slice->HasPlacement(layerId))
        {
            continue;
        }
        const auto cellWidth = std::max(1, slice->CellSize().width);
        ...
        for (auto column = columnBegin; column < columnEnd; ++column)
        {
            if (slice->PlacementCoversColumn(layerId, column))
            {
...
std::optional<til::point> AdaptDispatch::_resolveKittyPlacementAnchor(const uint32_t parentImageId, const uint32_t parentPlacementId, const std::pair<uint32_t, uint32_t> origin, std::wstring_view& code) const
{
    ...
    for (auto depth = 1; depth <= MaxKittyPlacementDepth; ++depth)
    {
        ...
        if (it == _kittyPlacements.end())
        {
            ...
            if (key.second == 0 && _kittyVirtualIds.count(key) != 0)
            {
...
int AdaptDispatch::_KittyPlaceholderDiacriticIndex(const char32_t ch) noexcept
{
...
bool AdaptDispatch::_placeKittyPlaceholderCell(const KittyImage& image, const uint32_t imageId, const til::CoordType column, const til::CoordType row, const uint32_t cellRow, const uint32_t cellCol, const KittyVirtualPlacement& place)
{
    ...
    const auto required = existingSlice && existingSlice->CellSize() == clampedCellSize ?
                              existingSlice->KittyWriteMemoryUpperBound(column, column + 1, place.zIndex, imageId, place.layerId) :
                              emptySlice.KittyWriteMemoryUpperBound(column, column + 1, place.zIndex, imageId, place.layerId);
    if (required > ImageSlice::KittyLayerBytesAvailable())
    {
        LOG_HR(E_OUTOFMEMORY);
        return false;
    }
    ...
    auto dstIterator = dstSlice->MutablePixels(column, column + 1, place.zIndex, imageId, place.layerId);
    auto sourceIterator = dstSlice->MutableSourceIndices(column, column + 1, place.zIndex, imageId, place.layerId);
    for (auto pixelRow = 0; pixelRow < cellHeight; ++pixelRow)
    {
    ...
    for (size_t i = 0; i < segment.size();)
    {
        ...
        if (i + 1 < next && segment[i] == KittyPlaceholderCodePointHigh && segment[i + 1] == KittyPlaceholderCodePointLow)
        {
            ...
            for (auto j = i + 2; j < next; ++j)
            {
                ...
                if (const auto idx = _KittyPlaceholderDiacriticIndex(cp); idx >= 0)
                {
                    if (rowDiacritic < 0)
                    {
                        rowDiacritic = idx;
