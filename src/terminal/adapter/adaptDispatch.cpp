...
void AdaptDispatch::_EraseAll()
{
    ...
    _deleteAllKittyPlacements(false);
}
...
ITermDispatch::StringHandler AdaptDispatch::KittyGraphics()
{
    return [this, control = std::wstring{}, payload = std::string{}, inControl = true, controlValid = true, payloadValid = true, payloadTooLarge = false](const auto ch) mutable noexcept -> bool {
        try
        {
            ...
            if (ch == AsciiChars::ESC)
            {
                _HandleKittyGraphics(control, payload, controlValid, payloadValid, payloadTooLarge);
                return false;
            }
            if (inControl && ch == L';')
            {
            ...
            if (inControl)
            {
                ...
                if (control.size() < MaxKittyControl)
                {
...
void AdaptDispatch::_HandleKittyGraphics(const std::wstring_view control, const std::string_view payload, const bool controlValid, const bool payloadValid, const bool payloadTooLarge)
{
    ...
    if (!controlValid)
    {
        ...
        if (command.quiet < 2)
        {
            _ReturnApcResponse(L"G;EINVAL:control block too long");
        }
        return;
    }
...
til::size AdaptDispatch::_placeKittyImage(const KittyImage& image, const bool moveCursor, const uint32_t imageId, const uint64_t layerId, const uint32_t cols, const uint32_t rows, const uint32_t srcX, const uint32_t srcY, const uint32_t srcW, const uint32_t srcH, const uint32_t cellOffsetX, const uint32_t cellOffsetY, const int32_t zIndex, const std::optional<til::point> anchor)
{
    ...
    if (drawnColumns > 0 && drawnRows > 0)
    {
        ...
        if (const auto entry = _kittyImages.find(imageId); entry != _kittyImages.end())
        {
...
bool AdaptDispatch::_placeImageCellRef(const KittyImage& image, const uint32_t imageId, const til::CoordType column, const til::CoordType row, const uint32_t cellRow, const uint32_t cellCol, const KittyVirtualPlacement& place)
{
    const auto framePixels = _kittyFramePixels(image, image.presentedFrame);
    if (!framePixels || framePixels->empty() || image.width == 0 || image.height == 0)
    {
