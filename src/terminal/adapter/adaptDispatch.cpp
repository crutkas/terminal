...
{
    return [this, control = std::wstring{}, payload = std::string{}, inControl = true, payloadValid = true, payloadTooLarge = false](const auto ch) mutable noexcept -> bool {
        try
        {
        ...
        if (i + 1 < next && segment[i] == KittyPlaceholderCodePointHigh && segment[i + 1] == KittyPlaceholderCodePointLow)
        {
            ...
            if ((fg.IsRgb() || fg.IsIndex256()) && idHighByte <= 255)
            {
                ...
                if (placement != _kittyVirtualIds.end() && imageEntry != _kittyImages.end())
                {
                    ...
                    if (_placeImageCellRef(imageEntry->second, imageId, column, screenRow, cellRow, cellCol, place))
                    {
