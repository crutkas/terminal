...
public:
    ...
    // These drive the vendored puff() inflater (oss/puff) through the private helper
    // (AdapterTest is a friend). Inputs are real zlib streams from zlib.deflateSync,
    // i.e. exactly what kitty transmits, so puff's actual Huffman decode is exercised.
    ...
    // which would catch any compressed/uncompressed size confusion in the two-pass decode.
    ...
    // An inflated size exceeding the cap is rejected (decompression-bomb guard via
    // puff's scanning pass) without allocating the expansion; the boundary (size == cap)
    // is accepted.
    ...
    // Trailing bytes between the DEFLATE stream and the Adler-32 are rejected (puff must
    // consume exactly the deflate body): this is c12 with an extra 0x00 spliced in before
    // the 4-byte checksum.
