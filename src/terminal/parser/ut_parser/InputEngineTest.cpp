...
class Microsoft::Console::VirtualTerminal::InputEngineTest
{
    ...
    // Runs a series of pipe reads through a fresh input engine and returns everything it
    // handed to the application's input buffer. See the definition for the details.
    static std::wstring RunInputSequences(const std::vector<std::wstring>& reads, const bool acceptC1 = false, const bool primeWin32InputMode = false);
...
// Collects everything the input engine hands to the application's input buffer as a
// single string. WriteStringRaw synthesizes one key-down record per character, which is
// exactly how a response reaches the client, so re-joining them recovers the sequence.
// primeWin32InputMode reproduces the real ConPTY configuration, where conhost has already
// requested win32-input-mode and the engine has seen a sequence confirming it.
std::wstring InputEngineTest::RunInputSequences(const std::vector<std::wstring>& reads, const bool acceptC1, const bool primeWin32InputMode)
{
    std::wstring delivered;
    auto pfn = [&](const std::span<const INPUT_RECORD>& records) {
        for (const auto& record : records)
        {
            if (record.EventType == KEY_EVENT && record.Event.KeyEvent.bKeyDown)
            {
                delivered.push_back(record.Event.KeyEvent.uChar.UnicodeChar);
            }
        }
    };

    TestState localState;
    localState._expectSendCtrlC = true; // WriteCtrlKey is how a win32-input-mode key arrives
    auto dispatch = std::make_unique<TestInteractDispatch>(pfn, &localState);
    auto inputEngine = std::make_unique<InputStateMachineEngine>(std::move(dispatch));
    StateMachine stateMachine{ std::move(inputEngine) };
    stateMachine.SetParserMode(StateMachine::Mode::AcceptC1, acceptC1);
    if (primeWin32InputMode)
    {
        stateMachine.ProcessString(L"\x1b[65;30;65;1;0;1_"); // an 'A' keydown in win32-input-mode
        delivered.clear();
    }
    for (const auto& read : reads)
    {
        stateMachine.ProcessString(read);
    }
    VERIFY_ARE_EQUAL(StateMachine::VTStates::Ground, stateMachine._state, L"the sequence must be fully consumed");
    return delivered;
}

void InputEngineTest::ApcResponseReachesTheApplication()
{
    // ConPTY deliberately does not answer requests itself: ConhostInternalGetSet::ReturnResponse
    // bails out in VtIo mode because "that's the job of the terminal". So under Windows Terminal,
    // the Kitty graphics acknowledgement is generated by the terminal and written back down the
    // ConPTY *input* pipe, where VtInputThread feeds it to this engine. Swallowing it (as an
    // unclaimed APC string was) leaves the application waiting forever for an answer to a query
    // it did receive an image for. DCS responses (DA3, DECRQSS) already survive this path; an
    // APC response has to as well.
    const std::wstring ack = L"\x1b_Gi=1;OK\x1b\\";
    VERIFY_ARE_EQUAL(ack, RunInputSequences({ ack }), L"a 7-bit APC response must reach the application unchanged");

    // Kitty allows an id-less acknowledgement, and an error response carries a payload with
    // characters (':', ' ') that no other part of the input engine has a meaning for.
    const std::wstring error = L"\x1b_GEINVAL:bad payload\x1b\\";
    VERIFY_ARE_EQUAL(error, RunInputSequences({ error }), L"an APC error response is passed through verbatim");
